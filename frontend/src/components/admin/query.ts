...
export interface AdminQueryOptions<T extends object> {
...
export function authorDisplayNames(
  ...
  authorsById: Record<string, AuthorRow>,
  labelOf: (a: AuthorRow) => string,
): string {
  let ids: string[];
...
export function edgeDisplayLabel(
  row: EdgeRow,
  worksById: Record<string, WorkRow>,
  labelOf: (w: WorkRow) => string,
): string {
...
export function applyAdminQuery<T extends object>(
  rows: T[],
