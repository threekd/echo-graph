...
- [x] **手工新增审核语义收口 + 并发一致性修复(审查跟进)**:API 手工新增/编辑
      ...
      测试缺陷(后端 286/286 全绿)。
- [x] **永久删除级联幂等(bug 修复)**:永久删除作品/作者会连带物理删除其涟漪/
      作品,随后再对已被级联删除的行执行永久删除改为幂等成功(响应含
      `already_deleted`,不再 404「未找到」);该 id 仍存在于他人空间时维持 404
      隔离语义;新增回归测试(后端 289/289 全绿)。
- [x] **书友卡片自动收起(交互)**:星际跃迁后右上角书友卡片展开 10 秒无鼠标停留
      自动向上收起为右上角小房子图标;点击图标重新展开,鼠标停留在卡片上
      保持展开。
- [x] **LLM 空正文自动重试(bug 修复)**:DeepSeek 深度思考模式偶发「只返回
      reasoning、正文为空」导致 `JSON 解析失败:Expecting value` 导入中断;
      `llm_client.call_json_completion` 对空正文/解析失败/网络异常做有界重试
      (2 次,指数退避),提取/补全阶段统一接入;上传目录默认迁到 `data/imports`
      (支持 `IMPORT_DIR` 覆盖),修复服务器检出目录不可写导致的
      `创建上传目录失败:Permission denied`。
