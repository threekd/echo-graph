...
var expandTimer = null;    // 扩散滑动条防抖计时器
...
// 拖动滑动条:数值即时更新,API 请求防抖 400ms(避免连续请求与视图闪烁)
function onExpandInput() {
  if (!rippleCenter) return;
  var hops = parseInt(el("expand-range").value, 10) || 1;
  el("expand-value").textContent = hops + " 级";
  if (expandTimer) clearTimeout(expandTimer);
  expandTimer = setTimeout(expandRipple, 400);
}
...
export function wireEvents() {
  ...
  el("expand-range").addEventListener("input", onExpandInput);
