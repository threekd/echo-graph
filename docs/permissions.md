...
> 互不审核;admin 整合用户数据进官方图谱的统一通道为后续规划。
> 公共星云概念已于 2026-08-27 移除:默认视图 = admin 星云(官方图谱),
> 功能栏「公共星云」标签保留,点击效果与跃迁到 admin 星云一致。
...
- **引导管理员**:`.env` 配置 `ADMIN_BOOTSTRAP_EMAIL` 的邮箱注册时自动获得 `admin` 角色,
  其星云即官方图谱(默认视图);启动/注册时对旧库遗留 owner_id 为空的行做一次性
  兼容认领(`app/auth.py` `bootstrap_admin`、`claim_unowned_rows`)。
- **游客**:未登录(无会话 Cookie)。
...
| 发布(审核批准 → 写入空间) | ❌ | ❌ | ✅ 发布到**自己的星云** | ✅ 发布到**自己的星云**(admin 即官方图谱) |
...
3. **多 admin 各自独立、互不审核**:草稿按 `owner_id` 隔离,审核接口只操作本人
   上传的草稿,跨上传者一律 404。
4. **admin 整合用户数据进官方图谱的通道 = 后续规划**:当前发布落点恒为上传者自己的星云;
   后续将新增「admin 审核 → 复制进官方图谱」的统一通道
   (涉及 `llm_review` 发布落点、判重目标,见 `docs/to-do.md`)。

## 6. 边界与注意点

- **官方图谱的特殊性(仅产品层面)**:引导管理员的星云即默认视图,其数据对游客可见;
  因此引导管理员审核批准的内容会直接进入官方图谱,与普通 admin / VIP 的发布落点不同。
- **VIP 发布不进官方图谱**:VIP 批准后的数据留在自己的星云,只有星云设为公开
