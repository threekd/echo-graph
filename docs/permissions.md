...
| 数据管理(公共星云 CRUD + 自动 CSV 导出,`/api/admin/data`) | ❌ | ❌ | ❌ | ✅ |
...
- **发布落点(已定型)**:批准时发布行以 `owner_id = 上传者(=审核者)id` 写入
  ...
  只有引导管理员发布时,落点才等于「公共星云」并触发公共 CSV 导出
  (`app/space_crud.py:164` `after_write` 仅在 `owner == admin_user_id()` 时导出);
  普通 admin / VIP 发布的数据留在各自星云,不进 CSV、不进公共图谱。
...
4. **发布到公共星云的通道 = 后续规划**:当前发布落点恒为上传者自己的星云;
   ...
   (涉及 `llm_review` 发布落点、判重目标与 CSV 导出触发条件,见 `docs/to-do.md`)。

## 6. 边界与注意点

- **公共星云的特殊性**:引导管理员的星云 = 公共星云,其数据进 CSV 且对游客可见;
